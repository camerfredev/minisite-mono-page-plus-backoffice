FontsManagment = {
	...
	importFonts() {
		...
		this.fontsList.forEach((font) => {
			if (!toImport.includes({name: font.title.name,
					local : font.title.local})) {
				toImport.push({name: font.title.name,
				local : font.title.local});
			}
			if (!toImport.includes({name: font.main.name,
					local : font.main.local})) {
				toImport.push({name: font.main.name,
					local : font.main.local});
			}
		})
		let importHtml = "<style type=\"text/css\">"
		toImport.forEach((font) => {
			importHtml += `
				@font-face {
				font-family: ${font.name};
				src:  url('public/fonts/customFonts/${font.local}-Regular.ttf')  format('truetype');
			}`
